...
# ---------------------------------------------------------------------------
# 학습 함수 — 실제 로직은 ml_core.py 에 있고, 여기서는 캐싱만 얹는다.
#
# Streamlit 은 위젯을 하나 건드릴 때마다 스크립트를 처음부터 다시 실행한다. 캐싱이 없으면
# 사이드바 필터를 움직일 때마다 모델이 전부 재학습된다(실측 100초+). 반대로 캐싱을
# ml_core 안에 두면 그쪽이 Streamlit 런타임 없이는 돌지 않아 테스트가 불가능해진다.
# 그래서 "무엇을 계산하는가"(ml_core)와 "언제 다시 계산하는가"(여기)를 나눠 둔다.
# ---------------------------------------------------------------------------
train_regression_models = st.cache_resource(
    show_spinner="회귀 모델 5종 학습 중...")(ml_core.train_regression_models)
train_classification_models = st.cache_resource(
    show_spinner="QA 분류 모델 2종 학습 중...")(ml_core.train_classification_models)
train_aggregate_model = st.cache_resource(
    show_spinner="척x공정 집계 모델 학습 중...")(ml_core.train_aggregate_model)
